...
flags.DEFINE_float('beta', 1, 'The hyperparameter that controls the weight of the human bias in the loss computation.', lower_bound=0)
...
def train(epoch, model, tokenizer, optimizer, lr_scheduler, dataloader, log_interval, device):
    ...
    with tqdm(total=len(dataloader)) as pbar:
        for texts, labels, durations in dataloader:
            optimizer.zero_grad()
            labels = labels.to(device)
            durations = durations.to(device)
            tokenized = tokenizer(texts, padding="max_length", truncation=FLAGS.truncation, return_tensors="pt", max_length=250)
            tokenized.to(device)
            outputs = model(labels=labels, output_attentions=True, **tokenized)

            prediction_loss = outputs.loss

            # combine different attention heads and we ignore the fist token - [cls]
            attentions = torch.sum(outputs.attentions[0], dim=1).sum(dim=1)[:, 1:outputs.attentions[0].shape[3]]
            # normalise the attention values
            attentions = attentions / torch.sum(attentions, dim=1, keepdim=True)
            # compute divergence of attentions from human attentions
            div_loss = F.cross_entropy(attentions, durations[:, 1:durations.shape[1]])
            # combine the losses
            total_loss = prediction_loss + div_loss * FLAGS.beta
            ...
            if batch_idx % log_interval == 0:
                ...
                with torch.no_grad():
                    ...
                    logging.info('Epoch: %d, batch: %d, total_train_loss: %.4f, pred_loss: %.4f, div_loss: %.4f, train_acc: %.4f', epoch, batch_idx,
                        total_loss.item(), prediction_loss.item(), div_loss.item(), accuracy['accuracy'])
            batch_idx += 1
