...
OUTPUT_DIR = "output/attention_data/paragraphs/"
...
def load_data(sentence_file, word_file):
    ...
    if "PARAGRAPH_ID" not in word_df.columns:
        word_df["PARAGRAPH_ID"] = word_df["WORD_ID"].apply(lambda s: "-".join(s.split("-")[:2]))

    sentence_paragraph_df = word_df[["WORD_ID", "SENTENCE_ID", "PARAGRAPH_ID"]]

    paragraphs = {}

    for paragraph_id in sentence_paragraph_df["PARAGRAPH_ID"].unique():
        mask = sentence_paragraph_df["PARAGRAPH_ID"] == paragraph_id
        sentences = []
...
def process_sample(paragraph, tokenizer, special_tokens=True):
    inputs = tokenizer.encode_plus(paragraph, return_tensors='pt', add_special_tokens=special_tokens)
    individual_tokens = tokenizer.batch_encode_plus(paragraph.split(), add_special_tokens=False)['input_ids']

    tokens_per_word = list(zip(paragraph.split(), [len(item) for item in individual_tokens]))
...
def create_df_from_sentence(paragraph_id, paragraph_df, word_df, model, mask, tokenizer, device, special_tokens=True):
    paragraph = paragraph_df.loc[paragraph_df['PARAGRAPH_ID'] == paragraph_id,'PARAGRAPH'].iloc[0]
    attention_scores = get_attention_scores(paragraph, model, tokenizer, device, special_tokens=special_tokens)
...
def extraction_loop(model_name, paragraph_df, word_df, device):
    ...
    for i, paragraph_id in paragraph_df["PARAGRAPH_ID"].iteritems():
        print(f"{i + 1} of {paragraph_df.shape[0]}")
        mask = word_df['PARAGRAPH_ID'] == paragraph_id
        if mask.sum() == 0:
            errors.append(f"{paragraph_id} - Zero!")
            continue

        try:
            df = create_df_from_sentence(paragraph_id, paragraph_df, word_df, model, mask, tokenizer, device)
            dfs.append(df)

        except Exception as e:
            errors.append(f"{paragraph_id} - {e}")
